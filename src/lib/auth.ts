...
export async function enableBiometricLogin(user: SessionUser) {
  ...
  if (!optionsResponse.ok) {
    throw new Error('Nao foi possivel iniciar o cadastro biometrico.');
  }
  ...
  if (!verifyResponse.ok) {
    throw new Error('Nao foi possivel validar o cadastro biometrico.');
  }
...
export async function loginWithBiometrics(emailInput?: string) {
  ...
  if (!optionsResponse.ok) {
    throw new Error('Nenhuma credencial biometrica encontrada para este usuario.');
  }
  ...
  if (!verifyResponse.ok) {
    throw new Error('Falha na autenticacao biometrica.');
  }
